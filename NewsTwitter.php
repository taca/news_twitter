...
class NewsTwitter extends Frontend
{
    ...
    public function cron()
    {
        ...
        $objNews = $this->Database->prepare("SELECT tl_news.*, tl_news_archive.twitterAuth, tl_news_archive.twitterParams, tl_news_archive.twitter_key, tl_news_archive.twitter_secret, tl_news_archive.jumpTo AS parentJumpTo FROM tl_news LEFT OUTER JOIN tl_news_archive ON tl_news.pid=tl_news_archive.id WHERE tl_news_archive.twitter='1' AND tl_news.twitter='1' AND twitterStatus='cron' AND published='1'")->limit(10)->execute($dc->id);
    ...
    private function twitter($twitter_key, $twitter_secret, $varAuth, $strStatus, $strUrl='', $strUrlParams='', $msg = '')
    {
        ...
        if ($code != 200)
        {
            $this->log('Error connecting to Twitter: ' . $code, 'NewsTwitter twitter()', TL_ERROR);
            return false;
        }
        ...
        if ($code == 200)
        {
            $this->log('Twitter posted ' . $msg, 'NewsTwitter twitter()',
                       TL_GENERAL);
            return true;
        }

        $error = print_r($connection->http_info, true);
        $this->log('Error posting to Twitter: ' . $code . ": " . $error,
                   'NewsTwitter twitter()', TL_ERROR);
        return false;
    }
